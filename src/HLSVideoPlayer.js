...
function HLSVideoPlayer({ video, refreshVideo, onClose }) {
  ...
  const constructPlayer = useCallback(() => {
    const player = playerRef.current;
    if (!player) {
      ...
      const player = (playerRef.current = videojs(
        videoElement,
        videoJsOptions.current,
        () => {
          videojs.log("player is ready");
          // You can handle player events here, for example:

          var audioCtx = new (window.AudioContext ||
            window.webkitAudioContext)(); // get access to audio context
  ...
  // useeffect seq no 3 -- compose/ dispose
  ...
  useEffect(() => {
    // Make sure Video.js player is only initialized once
    if (refreshVideo && refreshVideo.id === video.id) {
      if (!playerRef.current) {
        // The Video.js player needs to be _inside_ the component el for React 18 Strict Mode.
        constructPlayer();
      } else {
  ...
  }, [refreshVideo, video, constructPlayer]);
